...
firebase_admin.initialize_app(cred,{'databaseURL':"https://faceattendancerealtime-9fe36-default-rtdb.firebaseio.com/",
                                    'storageBucket':"faceattendancerealtime-9fe36.appspot.com"
})
    ...
    if faceCurFrame:
        for encodeFace,faceLoc in zip(encodeCurFrame,faceCurFrame):
            ...
            if matches[matchIndex]:
                y1, x2, y2, x1 = faceLoc
                y1, x2, y2, x1 = y1 * 4, x2 * 4, y2 * 4, x1 * 4  # Adjusting for the scaling
                bbox = 55 + x1, 162 + y1, x2 - x1, y2 - y1  # Correcting the bbox format
                # Draw rectangle on imgBackground
